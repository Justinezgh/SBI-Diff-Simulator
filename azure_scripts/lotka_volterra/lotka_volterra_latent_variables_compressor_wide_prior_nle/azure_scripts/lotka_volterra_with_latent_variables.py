...
# parser.add_argument('--out_params', type=str, dest='folder')
...
if ON_AZURE:
  ...
  run.parent.log('n_epochs', args.n_epochs)
  run.parent.log('n_steps', args.n_steps)
  run.parent.log('dimension', args.dimension)
  run.parent.log('bijector_layers_shape', args.bijector_layers_shape)
  run.parent.log('nf_layers', args.nf_layers)
  run.parent.log('n_components', args.n_components)
  run.parent.log('score_weight', args.score_weight)
  run.parent.log('model_seed', args.model_seed)
  run.parent.log('initial_learning_rate', args.initial_learning_rate)
  run.parent.log('bijector_layers_size', args.bijector_layers_size)
  #run.log('output_dir', args.output_dir)
  run.log('output_file', args.output_file)
else:
  print(args)
...
def get_batch(key, batch_size=1e5):
    ...
    (log_probs, samples), scores = get_samples_and_scores(model, key, batch_size=batch_size, score_type='conditional')
...
class SmoothNPE(hk.Module):
    def __call__(self, theta):
        ...
        # net = tfb.Chain([tfb.Invert(lotka_volterra_theta_bijector),
        #                                               tfb.Scale(30.),
        #                                               tfb.Shift(-0.5)])(theta)
...
# output_path = os.path.join(output_folder, 'params_nd.pkl')
...
#       #obs = calculate_observables(observation.reshape([-1,10,2]))
