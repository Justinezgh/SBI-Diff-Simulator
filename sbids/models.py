...
class AffineSigmoidCoupling(hk.Module):
    ...
    def __call__(self, x, output_units, **condition_kwargs):
        ...
        log_a = jax.nn.tanh(
          hk.Linear(output_units*n_components, name='l3')(net)
        ) * log_a_bound
        b = hk.Linear(output_units*n_components, name='l4')(net)
        c = jax.nn.sigmoid(
          hk.Linear(output_units * n_components, name='l5')(net)
        ) * (1 - min_density_lower_bound) + min_density_lower_bound
...
class ConditionalRealNVP(hk.Module):
    """Normalizing flow based on RealNVP using specified bijector functions."""
    def __init__(
      self,
      d,
      *args,
    ...
    def __call__(self, y):
        if self.d == 1:
            chain = tfb.Chain([
              (self.bijector_fn(
                y,
                name='b%d' % i
              )(x=None, output_units=self.d)) for i in range(self.n_layer)
            ])
        else:
            chain = tfb.Chain([
                  tfb.Permute(jnp.arange(self.d)[::-1])(
                    tfb.RealNVP(
                      self.d//2,
                      bijector_fn=self.bijector_fn(y, name='b%d' % i)
                    )
        ...
        nvp = tfd.TransformedDistribution(
              tfd.MultivariateNormalDiag(
                0.5*jnp.ones(self.d),
                scale_identity_multiplier=0.05
              ),
              bijector=chain)
